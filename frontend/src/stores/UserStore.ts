...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    const role = ref("");
    const interval = ref<NodeJS.Timeout | null>(null); // Specify type for interval ref
    ...
    async function refreshToken() {
      ...
      const path = "http://localhost:3333/refreshToken";
      await axios
        .get(path, config)
        .then((response) => {
          if (response.data) {
    ...
    function stopInterval() {
      if (interval.value !== null) {
        // Use type guard to check if interval.value is not null
